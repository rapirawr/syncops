...
<div class="max-w-xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        <a href="{{ route('projects.show', $project) }}" class="text-zinc-500 hover:text-zinc-300 transition-colors">
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
            </svg>
        </a>
        <h1 class="text-lg font-bold tracking-tight text-white uppercase font-mono tracking-wider">Modify Target: {{ $project->name }}</h1>
    </div>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="category" id="category" value="{{ old('category', $project->category) }}" placeholder="e.g. backend, gateway, client"
                               class="mt-1.5 block w-full rounded border border-white/10 bg-zinc-950 px-3 py-1.5 text-xs text-white placeholder-zinc-700 focus:border-indigo-500 focus:outline-none transition font-mono">
                        @error('category')
                            <p class="mt-1.5 text-[10px] font-mono text-status-critical-text">{{ $message }}</p>
                        @enderror
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div x-data="{
                                 open: false,
                                 status: '{{ old('status', $project->status ?? 'in_progress') }}',
                                 stages: {
                                     'planning': 'Planning',
                    ...
                    <div>
                        ...
                        <input type="number" name="progress" id="progress" required min="0" max="100" value="{{ old('progress', $project->progress) }}"
                ...
                <div>
                    <label for="status_note" class="block text-[10px] font-bold uppercase tracking-wider text-zinc-400 font-mono">Change Note (Optional)</label>
                    <input type="text" name="status_note" id="status_note" value="{{ old('status_note') }}" placeholder="e.g. Migration complete, going to next stage"
                           class="mt-1.5 block w-full rounded border border-white/10 bg-zinc-950 px-3 py-1.5 text-xs text-white placeholder-zinc-700 focus:border-indigo-550 focus:ring-1 focus:ring-indigo-550 focus:outline-none transition font-mono">
                </div>
            </div>

            <div class="pt-4 flex items-center justify-end gap-3 font-mono">
                <a href="{{ route('projects.show', $project) }}" class="rounded border border-white/10 bg-zinc-950 px-4 py-2 text-xs font-semibold text-zinc-400 hover:bg-zinc-900 transition">
                    Cancel
                </a>
